...
def format_prompt(user_data):
    """
    Format a prompt for the Gemini API based on user financial data
...
def _extract_gemini_text(response):
    ...
    # Newer versions expose `.text` helper
...
def query_gemini(prompt):
    ...
    Falls back to mock advice when the API is unavailable or not configured.
    """
    try:
        model = _configure_gemini()
        if not model:
            print("Gemini API key is not configured. Returning mock investment advice.")
            return generate_mock_investment_advice(prompt)

        print(f"Attempting to generate content with Gemini model {settings.GEMINI_MODEL}")
    ...
    except Exception as exc:  # Broad catch to surface friendly message and fall back
        error_msg = f"Error when calling Gemini: {exc}"
        print(error_msg)
        return generate_mock_investment_advice(prompt)
...
def format_investment_advice(raw_response):
    """
    Format the Gemini response for better readability and modern display
    ...
    clean_html = clean_html.replace('<ul>', '<ul class="advice-list">')
    clean_html = clean_html.replace('<ol>', '<ol class="advice-list">')
...
def generate_investment_advice_prompt(profile):
    """Generate a comprehensive prompt for the Gemini model based on user's financial profile"""
