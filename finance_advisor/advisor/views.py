...
from .utils import format_investment_advice, query_gemini
...
def investment_advice(request):
    ...
    try:
        ...
        messages.error(request, f'Error generating investment advice: {str(e)}')
        return redirect('dashboard')
...
def chat_with_advisor(request):
    ...
    try:
        ...
        # Query Gemini with the chat prompt
...
def generate_investment_advice(user, profile):
    ...
    # Query Gemini for advice
    from .utils import query_gemini
    raw_advice_content = query_gemini(enhanced_prompt)
